program main
  ...
  open(1,file='/private/var/www/sites/nucleosome/media/downloads/NuPoP_F/profile/freqL.txt')
  read(1,*) freqL1; close(1)
  open(1,file='/private/var/www/sites/nucleosome/media/downloads/NuPoP_F/profile/tranL.txt')
  do i=1,4; read(1,*) tranL1(i,:); end do; close(1)
  open(1,file='/private/var/www/sites/nucleosome/media/downloads/NuPoP_F/profile/tranL2.txt')
  do i=1,16; read(1,*) tranL2(i,:); end do; close(1)
  open(1,file='/private/var/www/sites/nucleosome/media/downloads/NuPoP_F/profile/tranL3.txt')
  do i=1,64; read(1,*) tranL3(i,:); end do; close(1)
  open(1,file='/private/var/www/sites/nucleosome/media/downloads/NuPoP_F/profile/tranL4.txt')
  do i=1,256; read(1,*) tranL4(i,:); end do; close(1)

  open(1,file='/private/var/www/sites/nucleosome/media/downloads/NuPoP_F/profile/147freqN.txt')
  do i=1,147; read(1,*) freqN1(i,:); end do
  close(1)
  open(1,file='/private/var/www/sites/nucleosome/media/downloads/NuPoP_F/profile/147tranN.txt')
  do i=1,584; read(1,*) tranN1(i,:); end do
  close(1)

  open(1,file='/private/var/www/sites/nucleosome/media/downloads/NuPoP_F/profile/146-149freqN4.txt')
  do i=1,64; read(1,*) freqN4(i,:); end do; close(1)
  open(1,file='/private/var/www/sites/nucleosome/media/downloads/NuPoP_F/profile/146-149tranN4.txt')
  do i=1,(147-4)*256; read(1,*) tranN4(i,:); end do; close(1)

  open(1,file='/private/var/www/sites/nucleosome/media/downloads/NuPoP_F/profile/Pd.txt')
...
subroutine vtbfb(lfn,filename,freqL,tranL,freqN,tranNN,mlL,rep,species,Pdd)

  implicit none
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  integer   t,d,i,j,k,l,m,n,z,mlL,lfn,rep,nvt,species,tep
  ...
  read(1,'(a)') a(1); m=len_trim(a(1)); k=1; j=m
  ...
  close(1)
  if(ztt<148) then
    print*,'The sequence length is shorter than 148bp, quit!'; stop
  ...
  out1(1:lfn)=filename; out1(lfn+1:lfn+16)='_Prediction1.txt'
  open(1,file=out1(1:lfn+16),status='replace'); temp=-0.05; t=0; tpc='NA'
  ...
  if(species<0.or.species>11) then
    print*,'unavailable for this species number, quit!'; stop
  end if
  ...
  if(l==rep) then
    ...
    open(1,file=out1(1:lfn+16),position='append')
  ...
  if(l==rep) then; open(1,file=out1(1:lfn+16),position='append')
    temp=-0.05; t=0
    if(x<nsec) then
...
subroutine vtbfbNL4(lfn,filename,freqL1,tranL1,TtranL2,TtranL3,TtranL4,TfreqN4,TtranN4,mlL,rep,species,Pdd)
  ...
  integer   t,d,i,j,k,l,m,n,z,mlL,rep,lfn,nvt,species,tep
  integer   zt,np,zp(100),q,st(100),ed(100),ext,rts(100),rte(100)
  integer   ztt,nsec,x,base; integer,allocatable:: nrcv(:),nbrk(:)
  ...
  out1(1:lfn)=filename; out1(lfn+1:lfn+16)='_Prediction4.txt'
  open(1,file=out1(1:lfn+16),status='replace'); temp=-0.05; t=0; tpc='NA'
  write(1,*) 'Position  P-start Occup  N/L  Affinity'
  do i=1,nrcv(1)-1; write(1,'(i9,2x,f5.2,3x,f5.2,3x,i1,4x,a)') i,temp,temp,t,tpc; end do
  close(1)
